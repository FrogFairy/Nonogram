...
Position Logic_board::col_changed(Position pos)
{
    ...
    {
        int row = std::find(col_intervals[y].begin(), col_intervals[y].end(), Position {up, down}) - col_intervals[y].begin();
        res {y, row};
    }
...
std::vector<Position> Logic_board::changed_digits(Position pos)
{
    ...
    res[0] = row_changed(Position{x, y});
    res[1] = col_changed(Position{x, y});
...
Position Logic_board::hint_click()
{
    ...
    current[pos.x]pos.[y] = correct[x][y] + 2;
