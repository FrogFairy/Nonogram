...
Choose_level_window::Choose_level_window(Graph_lib::Point xy, int w, int h, const std::string &title, Size size, Windows_wrapper &own)
    : Window_with_back{xy, w, h, title},
      level_widget{level_widget_point, level_widget_width, level_widget_height, Graph_lib::Menu::vertical, "widget_for_main_window"}, size{size}, own{own}
{
    Window_with_back::size_range(w, h, w, h);
    Graph_lib::Menu level_widget{level_widget_point, level_widget_width, level_widget_height, Graph_lib::Menu::vertical, "levels"};
