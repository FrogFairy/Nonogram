...
Main_window::Main_window(Graph_lib::Point xy, int w, int h, const std::string &title)
    ...
    {
        levels_menu.attach(new Graph_lib::Button{Graph_lib::Point{0,0}, 10, 10, "10x10", cb_go_to_level});
        attach(levels_menu);
        levels_menu.attach(new Graph_lib::Button{Graph_lib::Point{0,0}, 10, 10, "15x15", cb_go_to_level});
        attach(levels_menu);
        levels_menu.attach(new Graph_lib::Button{Graph_lib::Point{0,0}, 10, 10, "20x20", cb_go_to_level});
        attach(levels_menu);
    };

Main_window::Main_window(Graph_lib::Point xy, int w, int h, const std::string &title)
    : Graph_lib::Window{xy, w, h, title},
    generate_button{Graph_lib::Point{260, 250}, 200, 50, "Сгенерировать уровень", cb_create_level}
    {
